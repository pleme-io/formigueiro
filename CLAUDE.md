...
| **`formigueiro-flake`** | The **`flake-input` Environment** — the first *real* `UpdateEnv`. A typed parse of `flake.lock` (`FlakeLock` → current locked rev) + a mockable `RefResolver` (upstream head via `git ls-remote`, **typed argv, no shell**) composed into `FlakeEnv`. The `RefResolver` trait IS the TYPED-SPEC triplet's Environment seam — real network never touches a unit test. Typed errors (`FlakeError` + `Display`, **no `format!()`**), no panic/unwrap. Proven: a swarm shadows a real `blackmatter` bump end-to-end (frozen → held, `pending_plan` surfaces the concrete `to` rev). | formigueiro-core, serde_json |
