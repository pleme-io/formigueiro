...
| **`formigueiro-image`** | The **second update domain** — proof the core is domain-agnostic. The `image-tag` Environment: `ImageEnv` (current = the pinned digest from the caller's pin set; latest = a mockable `DigestResolver` → `SkopeoDigestResolver`, `skopeo inspect`, typed argv no shell) + `ImageSignalSource`. Uses the generic `DiffKind::new("image-tag")` — *nothing* re-implements shadow/promotion/pacing/swarm; it supplies only the two seams. Same rigor as `formigueiro-flake` (typed `ImageError`+Display, no `format!`). Proven: a swarm shadows an image digest bump end-to-end via the *same core*. | formigueiro-core, serde |
| **`formigueiro-cargo`** | The **third update domain**: the `cargo-dep` Environment. `CargoEnv` (current = pinned crate version; latest = a mockable `CrateResolver` → `CargoSearchResolver`, `cargo search`, typed argv no shell) + `CargoSignalSource`, on `DiffKind::new("cargo-dep")`. Three domains confirm the core is a template, not a coincidence. | formigueiro-core, serde |
...
- **The kind catalog self-describes (CATALOG REFLECTION).** Every update kind has a
  typed `KIND_CATALOG` entry (name/domain/maturity/purpose); adding a kind requires
  its entry — the reflection tests (unique names, the shadow **matrix**, maturity
  partition) fail otherwise, so code and self-description can't drift. `formigueiro
  --list-kinds` emits it. New diff-domains reuse the generic `DiffKind`; each supplies
  only an `UpdateEnv` + `SignalSource` + a `Resolver` (flake / image / cargo are the
  worked examples).
